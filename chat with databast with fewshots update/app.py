...
st.title("Chat with Postgres DB (with Few-Shot Examples)")
...
@st.cache_resource
def load_fewshots():
    fewshots_path = os.path.join(os.path.dirname(__file__), "fewshots.json")
    with open(fewshots_path, "r", encoding="utf-8") as f:
        return json.load(f)


def build_fewshot_prompt(db):
...
def get_sql_query(question):
    db = get_db()
    llm = get_llm()
    prompt = build_fewshot_prompt(db)
    chain = prompt | llm | StrOutputParser()
    raw = chain.invoke({"input": question, "table_info": db.get_table_info()})
